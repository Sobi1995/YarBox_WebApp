...
export class authenticationService {
  ...
  checkVerifyCode(verifycode: VerifyCode): Observable<JwtFormat> {
    ...
    headers = headers.set("Content-Type", "application/json; charset=utf-8");
    return this._http
      .post<JwtFormat>(this.api + "account/verify", verifycode, {
        headers: headers
      })
      .pipe(x => x);
  }
  ...
  LogOut() {
    ...
    headers = headers.set("Content-Type", "application/json; charset=utf-8");
    return this._http
      .get(this.api + "account/sign-out", { headers: headers })
      .pipe(
        map(response => {
          // localStorage.clear();
  ...
  getCheck(): Observable<void> {
    ...
    return this._http
      .get(this.api + "account/check", { headers: headers })
      .pipe(
        map(response => {
          this._webAppService.setLoding(false);
  ...
  IsLoginOnServer(): Observable<any> {
    ...
    return this._http
      .get(this.api + "account/isLogin", { headers: headers })
      .pipe(
        map((response: any) => {
          this._webAppService.setLoding(false);
  ...
  Register(user) {
    ...
    headers = headers.set("Content-Type", "application/json; charset=utf-8");
    return this._http
      .post(this.api + "account/register ", user, { headers: headers })
      .pipe(
        map(
          (response: Response) => {
            this._webAppService.setLoding(false);
